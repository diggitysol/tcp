	...
	*/
	public function insert($collection = "", $insert = array())
	{
		...
		try
		{
			...
			$this->db->{$collection}->insert($insert, array('w' => $this->write_concerns));
	...
	*/
	public function batch_insert($collection = "", $insert = array())
	{
		...
		try
		{
			...
			$this->db->{$collection}->batchInsert($insert, array('w' => $this->write_concerns));
			if(is_array($insert) && count($insert) > 0)
			{
	...
	*/
	public function update($collection = "", $options = array())
	{
		...
		try
		{
			...
			$options = array_merge(array('w' => $this->write_concerns, 'multiple' => FALSE), $options);
	...
	*/
	public function update_all($collection = "", $data = array(), $options = array())
	{
		...
		try
		{
			...
			$options = array_merge(array('w' => $this->write_concerns, 'multiple' => TRUE), $options);
	...
	*/
	public function delete($collection = "")
	{
		...
		try
		{
			...
			$this->db->{$collection}->remove($this->wheres, array('w' => $this->write_concerns, 'justOne' => TRUE));
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
	...
	*/
	public function delete_all($collection = "")
	{
		...
		try
		{
			...
			$this->db->{$collection}->remove($this->wheres, array('w' => $this->write_concerns, 'justOne' => FALSE));
			$this->_clear();
			return (TRUE);
		}
		catch (MongoCursorException $e)
