...
class User {
	...
	public function __construct(){
		...
		if ($this->CI->session->userdata('user_id')) {
			//print_r( new MongoId($this->CI->session->userdata('user_id')));
			$user_query=$this->CI->mongo_db
									->where('_id',new MongoId($this->CI->session->userdata('user_id')))
									->where('user_group_id',1)
									->get('user');
									
			if($user_query){
				$res = (object) $user_query[0];
				
				$this->user_id 		= $res->_id->{'$id'};
				...
				$user_group_query=$this->CI->mongo_db
									->where('user_group_id',$this->user_group_id)
									->get('user_group');
		
				$groups = (object) $user_group_query[0];
	...
	public function login($username, $password) {
		...
		if ($user_query)
		{
			...
			if (!$res->activated){
				...
				//echo $res->_id->{'$id'};
				$this->CI->session->set_userdata('user_id', $res->_id->{'$id'});
	...
	public function checkLogin() {
		...
		if($this->CI->uri->total_segments() == 2 || $this->CI->uri->total_segments() == 3)
		{
	...
	public function getImage() {
		...
		if ($this->image && is_file(DIR_IMAGE . $this->image)) {
