...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _log_request($authorized = FALSE)
    {
			...
			if(config_item('rest_use_mongodb')){
				$is_inserted=$this->mongo_db->insert(config_item('rest_logs_collection'), $log) ;
				$this->_insert_id = $is_inserted;
			}else{
				$is_inserted=$this->rest->db->insert(config_item('rest_logs_table'), $log) ;
		  ...
		  $payload['rtime'] = $this->_end_rtime - $this->_start_rtime;
			if(config_item('rest_use_mongodb')){
				return $this->mongo_db
					->where('_id', $this->_insert_id)
					->set('rtime', $this->_end_rtime - $this->_start_rtime)
					->update(config_item('rest_logs_collection'));
    ...
    protected function _check_access()
    {
        ...
        //check if the key has all_access
		  if(config_item('rest_use_mongodb')){
				$accessRow= $this->mongo_db
					->where('key', $this->rest->key)
					->get(config_item('rest_access_collection'));
