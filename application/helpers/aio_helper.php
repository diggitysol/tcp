      ...
      $CI->load->library('image_lib');
		...
		if (!is_file(DIR_IMAGE . $image_new) || (filectime(DIR_IMAGE . $image_old) > filectime(DIR_IMAGE . $image_new))) {
			...
			if ($width_orig != $width || $height_orig != $height) {
				$config = array(
					'source_image' 	=> DIR_IMAGE . $image_old ,//get original image
					'new_image' 		=> DIR_IMAGE . $image_new, //save as new image //need to create thumbs first
					'maintain_ratio' 	=> true, 
					'width' 				=> $width,
					'height' 			=>$height   
				);
...
if ( ! function_exists('clear_image'))
{
	function clear_image($dir,$filename) 
   {
		if (file_exists($filename)) {
			@unlink($file);
		}
		if ( file_exists(DIR_IMAGE . IMAGE_CACHE . '/uploads/'.$dir) ) 
      {
			foreach (glob(DIR_IMAGE . IMAGE_CACHE . '/uploads/'.$dir . '/*') as $file)
         {
            @unlink($file);
         }
		}
...
if ( ! function_exists('front_url'))
{
    ...
    {
			$CI =& get_instance();
			return $CI->config->item('front_url').trim($uri, '/');
    }
}
...
if ( ! function_exists('tz_list'))
{
	function tz_list()
	{
		...
		{	
			...
			if (isset($country[1]) != ''){
				$text = str_replace("_"," ",$country[1]);
			}else{
				$text = str_replace("_"," ",$country[0]);
			}
			$zones_array[$country[0]][$zone] = $text.' ('.$hours.':'.$mins.')';
		}
		return $zones_array;
	}
}
...
if (!function_exists('load_admin_controller'))
{
    function load_admin_controller($controller, $method = 'index')
    {
